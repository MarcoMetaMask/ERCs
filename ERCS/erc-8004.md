...
```jsonc
{
  ...
  "endpoints": [
   ...
   {
      "name": "agentWallet",
      "endpoint": "{namespace}:{chainId}:{address}" // e.g. for Mainnet: eip155:1:0x331...
    },
    {
      "name": "A2A",
...
When the agent NFT is transferred, `agentWallet` is automatically reset to the zero address and must be re-verified by the new owner.
