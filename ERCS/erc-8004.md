...
```javascript
function giveFeedback(uint256 agentId, uint8 score, string tag1, string tag2, string calldata feedbackUri, bytes32 calldata feedbackHash) external
```
...
```javascript
event NewFeedback(uint256 indexed agentId, address indexed clientAddress, uint8 score, string indexed tag1, string tag2, string feedbackUri, bytes32 feedbackHash)
```
...
This function MUST be called by the owner or operator of *agentId*. The *requestUri* points to off-chain data containing all information needed for the validator to validate, including inputs and outputs needed for the verification. The *requestHash* is a commitment to this data, and it's used as unique identifier for the request. All other fields are mandatory.
...
function getSummary(uint256 agentId, address[] calldata validatorAddresses, string tag) external view returns (uint64 count, uint8 avgResponse)
